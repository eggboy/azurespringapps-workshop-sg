This unit will create persistent stores (postgres, Azure Cache for Redis) outside the applications and connect applications to those stores. The arm template that was executed in [03-workshop-environment-setup](../01-workshop-environment-setup/acmedeploy.json) created the Postgres Server instance and Azure Cache for Redis instance.
...
Open `./scripts/setup-db-env-variables.sh` and change the XX to your assigned number:
